...
class TestGetInfo:
    def test_returns_info_dict(self, provider, apple_info, mock_yf_ticker):
        with patch("dashboard.providers.yahoo_provider.yf.Ticker", return_value=mock_yf_ticker):
    ...
    def test_raises_ticker_not_found_when_empty(self, provider):
        mock = MagicMock()
        mock.info = {}
        with patch("dashboard.providers.yahoo_provider.yf.Ticker", return_value=mock):
    ...
    def test_raises_data_fetch_error_on_exception(self, provider):
        ...
        with patch("dashboard.providers.yahoo_provider.yf.Ticker", side_effect=RuntimeError("net")):
...
class TestGetPriceHistory:
    ...
    def test_raises_data_fetch_error_on_exception(self, provider):
        with patch(
            "dashboard.providers.yahoo_provider.yf.Ticker",
